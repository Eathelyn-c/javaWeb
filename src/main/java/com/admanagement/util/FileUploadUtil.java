...
public class FileUploadUtil {
    ...
    private static String uploadFile(Part part) throws Exception {
        ...
        if (REMOTE_UPLOAD_ENABLED) {
            ...
            // Save to local storage when remote upload is disabled
            ...
            if (!uploadDir.exists()) {
                uploadDir.mkdirs();
                System.out.println("Created upload directory: " + STORAGE_ROOT);
            }
    ...
    private static boolean trySystemScpUpload(String localFilePath, String fileName) {
        try {
            ...
            // Build scp command
            // scp -o StrictHostKeyChecking=no -o PreferredAuthentications=password localFile user@host:remotePath
            ...
            System.out.println("Executing: scp " + localFilePath + " -> " + remoteDest);
            
            // Start process with password input
            Process process = pb.start();
            
            // Send password to scp process
    ...
    private static void uploadToRemoteServer(String localFilePath, String fileName) throws Exception {
        ...
        // Fallback to JSch if scp is not available
